 ...
 *
 * `frameP95` в 60 мс выглядит громадным, и это правда: 28.08.2026 главная
 * на этой скорости даёт p95 = 50 мс, то есть каждый двадцатый кадр выходит
 * втрое позже срока. Это записанный долг, а не разрешение — см. `docs/PLAN.md`.
 * Ворота ловят ухудшение относительно сегодняшнего дня; опустить порог —
 * отдельная работа, и опускать его надо вместе с починкой, иначе CI
 * покраснеет на коде, который никто не менял.
 */
