...
const RULES = [
  ...
  {
    /*
     * Ловим любой атрибут на «on», а не список известных имён: перечисление
     * пропустило бы `ontoggle`, `onpointerdown` и каждое новое событие
     * платформы. Атрибутов на «on», кроме обработчиков, в HTML нет,
     * а `data-onclick` под правило не попадёт — перед «on» нужен пробел.
     */
    name: 'обработчик on*=',
    re: /\son[a-z]+\s*=\s*["']/gi,
