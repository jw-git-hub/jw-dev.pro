...
Объявлены в `:root`. Три токена акцента — `--acc`, `--acc-ink` и `--acc2` — до
28.08.2026 регистрировались через `@property`, чтобы их можно было **анимировать**
при смене секции. Перетекание снято (отступление в §2), и регистрация снята вместе
с ним: замер показал, что без неё посчитанные цвета не меняются ни в одном из трёх
движков.
...
| `--hdr` | `52px` | высота шапки, одна на всей прокрутке |
...
Акцент сменяется мгновенно — см. отступление ниже. Одновременно перекрашиваются:
...
### Отступление: акцент без перетекания (28.08.2026, замер)

Гайд просил менять акцент плавно за `.9s`; ревизия производительности 26.08 срезала
это до `.3s`, а 28.08 перетекание снято совсем — `--acc`, `--acc-ink` и `--acc2`
меняются мгновенно.

Причина измерена, а не предположена. Три токена наследуются, поэтому «плавно» означает
новое значение на корне **каждый кадр**, а каждое новое значение на корне помечает
устаревшим стиль всей страницы: **1420 элементов, 25 мс на кадр** при вчетверо
замедленном процессоре. Акцент меняется одиннадцать раз за прокрутку главной, и это
съедало около двухсот кадров из шестисот. Медиана кадра при этом оставалась здоровой
(16,7 мс) — рвался хвост:

| прокрутка | с перетеканием, p95 | без него, p95 |
| --------- | ------------------- | ------------- |
| спокойная, 800 px/с | 33,9 мс | 17,6 мс |
| обычный свайп, 2500 px/с | 50,0 мс | 18,5 мс |
| резкий флик, 5000 px/с | 50,5 мс | 32,7 мс |

Укоротить вместо того, чтобы снять, не помогает: один кадр полного пересчёта стоит 25 мс,
поэтому переход любой длины оставляет рывки в хвосте (150 мс дают p95 = 35,2 мс).
Плавность дороже всего стоит ровно тогда, когда её не видно: при быстрой прокрутке
она не успевает доиграть до следующей смены акцента.

Решение владельца от 28.08.2026. Держат ворота `check:runtime`: тогда — порогом
`frameP95` в 30 мс, с 01.09.2026 — счётом пересчитанных элементов на кадр, порог 150.
Порог на время пришлось снять: он квантован развёрткой и краснел от загрузки машины,
а не от страницы. Цифры в таблице выше — замер того дня, они не пересняты и остаются
историей правки. Возвращённое на пробу перетекание даёт сегодня 293 элемента на кадр
против 60 — ворота его ловят.
...
| В шапке | `.btn.hdr-cta` | контур: `padding:8px 16px`, `13px/600`, рамка `color-mix(in srgb,var(--acc-ink) 42%,transparent)`, подсветка `…8%`, текст `--acc-ink`, иконка `arr` 14px; ховер — заливка `--acc-ink`, текст `#05070E`, рамка прозрачная |
...
## 9. Фон: плата с процессором

Канвас `#graph` во весь вьюпорт, `position:fixed`, `pointer-events:none`, `opacity:.66`, `z-index:2`.

### Отступление от макета: плата вместо сети (04.09.2026, решение владельца)

В макете фон — живая сеть: узлы дрейфуют, ближние соединяются рёбрами, а элементы
интерфейса с `data-nx` висят в ней на собственных нитях. Владелец забраковал её целиком
и попросил плату: плоские дорожки, немного, без объёма, и импульсы цветами сайта.

Вместе с сетью ушли **якоря**: `data-nx`, `data-nxc`, `data-nxg`, `data-nxp` сняты
с окон героя, HUD-пилюль и знака, а `graph/anchors.js` и `graph/field.js` удалены.
Решение сознательное: дорожка под 90° не может «подойти» к произвольному блоку вёрстки,
не превратившись в ту же сеть, только дороже. Раздел «связь элементов интерфейса»
из брифа закрыт отказом, а не реализацией.

### Композиция

**Процессор в центре вьюпорта**, дорожки сходятся к его выводам с четырёх краёв экрана.
Корпус — квадрат стороной 182px (112px при `GW<760`), центр и сторона посажены на сетку
`GRID = 26px`. Выводов по пять на сторону (три на узком), шаг между ними кратен сетке:
вывод обязан стоять в её узле, иначе диагональ дорожки придёт в него под 44°.

Рисунок корпуса: заливка альфой `.14`, рамка `1.4px` альфой `.85`, кристалл внутри —
квадрат в `.52` от полустороны, обводка `1px` альфой `.45`; выводы — штырьки `9×3`
наружу от кромки; метка первого вывода — точка `r=2.4` в 13px от левого верхнего угла.

### Дорожки

Одна дорожка на вывод, и приходит она с ближайшего к нему края экрана. Маршрут всегда
один и тот же: прямой участок от кромки, один поворот под 45° на строку вывода, снова
прямой и вход в вывод прямой шейкой `LEAD = 26px` по нормали.

Уступ диагонали — не больше пяти клеток и не больше половины свободного места.
**Потолок обязателен:** без него диагональ вырастает во весь экран и читается как
случайная линия поперёк страницы, а не как обход препятствия на плате.

Линия `1px` альфой `.42`, на изломах с вероятностью `.5` — переходное отверстие
`r=1.7` альфой `.55`. Ни свечения, ни градиентов: фон плоский.

### Два цвета, и они говорят о разном

**Дорожки и импульсы** держат акцент читаемой секции: по фону видно, где посетитель.
**Корпус процессора** перебирает всю палитру по кругу — `cyan → indigo → violet → amber
→ rose`, 3400 мс на переход, без пауз на чистом цвете (пауза читается как «зависло»,
а не как «работает»). Прокрутке он не подчиняется: он один на странице и показывает,
что плата под током.

Смена цвета дорожек — **мгновенная**, без перетекания. То же правило, что у CSS-акцента
(§2, отступление «Акцент без перетекания»), и по той же причине, только здесь она жёстче:
каждый промежуточный цвет означает перепечь полноэкранный слой заново. Замер прокрутки
главной: с перетеканием пропущенных кадров 15,2%, без него 10,7% на той же машине.

### Как это устроено в кадре

Геометрия дорожек от времени не зависит, поэтому она запекается **белой маской**
в закадровый холст один раз на размер окна. Второй закадровый холст держит ту же маску
в цвете текущей секции: он пересобирается композицией `source-in`, только когда цвет
и правда сменился, а в остальные кадры просто копируется одним `drawImage`.

Порядок кадра снизу вверх: окрашенная разводка, корпус, импульсы, дыры под текстом.

### Импульсы

Идут вдоль дорожки от кромки экрана к процессору со скоростью 1,5–2,8 px за кадр;
дойдя до вывода, гаснут. Рождаются не чаще одного в 900 мс, живых не больше пяти.

Голова — точка `r=1.9` альфой `.95` плюс гало `r=5` альфой `.16`. Хвост длиной 54px
разбит на шесть звеньев с убывающей альфой от `.50`; звенья берутся с самой дорожки,
поэтому хвост повторяет её изломы, а не срезает по прямой. Градиента вдоль пути нет
намеренно: градиент канваса живёт в экранных координатах, и его пришлось бы
пересоздавать каждый кадр для каждого импульса.

### Демпфирование под текстом

Фон не спорит с текстом: под каждым блоком с атрибутом `data-damp` в канвасе прожигается
мягкая дыра режимом `destination-out`. Значение атрибута — сила от 0 до 1, пустой атрибут
означает `.74`. Радиус — габариты блока плюс 30px по X и 26px по Y. Градиент строится
в единичном круге и растягивается матрицей, поэтому кешируется по силе: их на странице три.

Геометрия дыр пересчитывается раз в 560 мс (1120 мс при `GW<900`), а не каждый кадр:
`getBoundingClientRect` заставляет браузер пересчитать layout, и делать это шестьдесят раз
в секунду ради блоков, которые стоят на месте, — самый дорогой способ ничего не изменить.

### Производительность и паузы

`DPR = Math.min(1, devicePixelRatio)` (см. отступление «Бюджет кадра»), единственный
`requestAnimationFrame`-цикл, фон **не** параллаксится. На `GW<900` холст перерисовывается
30 раз в секунду вместо 60, а импульсы двигаются покадрово: пять сложений стоят
несравнимо меньше одного прохода по холсту во весь экран.

Останов: `document.hidden` (`visibilitychange`) и `prefers-reduced-motion` — при нём цикл
не заводится вовсе, рисуется один статичный кадр (`staticFrame()`): разводка и корпус
без импульсов. Убирать заодно и дорожки нельзя: §5 требует видимой структуры под стеклом,
и они — единственное, что её даёт.

Публичного `window.__graph` нет: модули связаны импортами, акцент зовёт `setTint(name)`
напрямую. `remeasure()` не понадобился — геометрия дыр пересчитывается по таймеру сама.
...
**Шапка** `position:fixed`, `z-index:50`, высота `var(--hdr)` = 52px **на всей прокрутке** —
ужимания нет. Сплошная, не стеклянная: `::before` — заливка
`linear-gradient(180deg,var(--panel),var(--panel2))`, блик `inset 0 1px 0 rgba(255,255,255,.08)`,
нижняя граница `color-mix(in srgb,var(--acc-ink) 32%,transparent)`; `::after` — шум `.025`.
`backdrop-filter` у шапки нет вовсе. Класс `.stuck` (при `scrollY > 18`) добавляет одну тень
`0 18px 40px -34px`. Пункт нав подчёркивается линией `scaleX(0)→scaleX(1)` цвета `--acc`
за `.45s`; активный пункт получает класс `.cur` при скролле.
Знак в шапке — 26px, пункты нав — 13px с отступом `7px 12px`.

### Отступление от макета: сплошная шапка (04.09.2026, решение владельца)

В макете шапка остаётся стеклянной на всей прокрутке, и в `.stuck` меняется только
прозрачность псевдоэлементов. На живом сайте это читалось как брак: заливка в 4% белого
контент не прячет, а размывает, и под шапкой ползут призраки заголовков и картинок.
Размытие — не подложка, оно перераспределяет пиксели, а не закрывает их.

Поэтому в `.stuck` заливка `::before` подменяется **непрозрачной**:
`linear-gradient(180deg,var(--panel) 0%,var(--panel2) 100%)`. Прозрачности не оставлено
ни процента, и это замер, а не осторожность: `backdrop-filter` доезжает до Safari
и не доезжает до Chrome и Firefox (см. техдолг в `docs/PLAN.md`), поэтому крыть обязана
одна заливка. Промежуточные 86% и 96% пробовались — текст под баром вычитывался
при увеличении в обоих случаях. Плоской полоса не выглядит: глубину даёт градиент,
рамку — блик сверху и линия акцента снизу.

Нижняя граница в `.stuck` красится акцентом секции
(`color-mix(in srgb,var(--acc-ink) 32%,transparent)`) вместо белых 8%: линия перетекает
вместе с содержимым, и шапка читается как часть страницы, а не планка поверх неё.
Переход добавлен к `background` и `border-color` рядом с `opacity`, те же `.5s var(--e)`.

Первой правкой в тот день шапку сделали плотной только в `.stuck`, а наверху оставили
прозрачной. На живом сайте это тоже оказалось браком, и владелец забраковал его сразу:
на первом экране заголовок героя проходил ровно за пунктами меню, и читались оба плохо.
Прозрачной шапке не помогает ни плотность, ни блюр — ей мешает то, что под ней контент.

Поэтому шапка сплошная **всегда**, одной высоты, без ужимания: 70 → 52px. Ориентир —
тонкая тёмная полоса старого apple.com. `backdrop-filter` снят с неё совсем: сплошной
полосе размывать нечего, а это был самый дорогой слой страницы. Заодно исчез обходной
приём `.hdr:has(.mnav-holder[open])`, который отдавал блюр шторке.

Знак ужат 34 → 26px, пункты нав 14 → 13px, кнопка — 6/14px и 12,5px.
...
**Знак `>jw_`** — вставляется дословно, четыре места: шапка (26px, ужат вместе с баром),
...
| Что | Длительность |
|---|---|
| Смена акцента `--acc`/`--acc2` | мгновенно (см. отступление «Акцент без перетекания», §2) |
...
**§2 и §16, переход акцента: 900 мс → 300 мс.** (28.08.2026 снят совсем —
см. отступление «Акцент без перетекания» в §2: замер показал, что дело
не в длительности, а в самом факте перехода.) `--acc`, `--acc-ink` и `--acc2` наследуются,
