...
--hdr:70px;
transition:--acc .3s var(--e), --acc-ink .3s var(--e), --acc2 .3s var(--e);
}
```
...
Переход между акцентами плавный за счёт `transition:--acc .3s var(--e)` на `:root` (работает
...
**На ≤900px размытия нет ни у одного из них, кроме `.mnav`.** Вместо него — та же плотная
заливка `rgba(13,17,32,.72)`, что и у `html.noblur`. Причина в отступлении «Бюджет кадра»:
под стеклом лежит канвас графа, и каждый его кадр требует пересчитать размытие заново.
Шторка блюр сохраняет — пока она открыта, страница не прокручивается. Тот же запасной
вариант включается через `@supports`, если `backdrop-filter` не поддержан.
...
**Производительность и паузы.** `DPR=Math.min(1,window.devicePixelRatio||1)` (см. отступление
«Бюджет кадра»); единственный
...
по вертикали. Положение ставит кадр анимации (`process.js`), поэтому перехода у него нет:
CSS-переход по свойству, которое переписывают шестьдесят раз в секунду, только
перезапускается и отстаёт.
...
| Что | Длительность |
|---|---|
| Смена акцента `--acc`/`--acc2` | 300 мс (см. отступление «Бюджет кадра») |
...
| Пересчёт геометрии графа | каждые 560 мс, на ≤900px — 1120 мс |
...
| Кольцо прогресса в доке | без перехода: смещение ставит кадр прокрутки |
...
| **≤900px** | герой в одну колонку с `.hero-l{order:-1}`; сцена статична и плоская (`transform-style:flat`), каустики скрыты, HUD в ряд; процесс — вертикальная рельса вместо SVG; кольцо стека выключается (`sync()` требует `innerWidth>900`); блюр стекла заменён плотной заливкой (§5); зерно во весь экран снято; пятна ауроры плывут без `scale`; граф рисуется 30 раз в секунду |
...
*,*::before,*::after{animation:none!important;scroll-behavior:auto!important;
  transition-property:opacity,color,background,background-color,border-color,box-shadow,fill,stroke!important}
:root{transition-property:--acc,--acc-ink,--acc2!important}
.rv{transform:none!important}
#light{display:none}
.marq-in{transform:none}
.win,.hud,.stage-in,.plane{transform:none!important}
.acc-i::details-content{transition:none!important}
}
```
...
### Отступление: бюджет кадра (26.08.2026)

Сайт лагал при прокрутке на Android и на слабом ПК. Замер (1440×900, троттлинг CPU ×4,
шесть секунд прокрутки главной) дал медиану кадра **67,8 мс** — пятнадцать кадров в секунду
вместо шестидесяти. Ниже — числа гайда, которые пришлось поменять, и почему.

**§9, плотность растра графа: `min(2,dpr)` → `min(1,dpr)`.** Канвас растянут во весь экран,
и удвоение плотности учетверяет число пикселей, которые нужно очистить и залить каждый кадр.
Это оказалось самой дорогой строкой всего сайта: одна замена дала **67,8 → 17,0 мс**.
Сеть — линии в один пиксель под непрозрачностью `.66`; чёткости, за которую платят вчетверо,
на них не видно.

**§2 и §16, переход акцента: 900 мс → 300 мс.** `--acc`, `--acc-ink` и `--acc2` наследуются,
и смена любого из них на `:root` помечает устаревшим стиль каждого элемента страницы —
на главной их больше тысячи. Всё время перехода браузер пересчитывает дерево целиком,
и на главной это происходит одиннадцать раз за прокрутку.

**§16, кольцо прогресса в доке: `.25s linear` → без перехода.** `dock.js` пишет
`stroke-dashoffset` каждый кадр прокрутки. Переход по свойству, которое переписывают
шестьдесят раз в секунду, не сглаживает его, а перезапускается и отстаёт на 250 мс.
То же самое было у точки рельсы процесса (§11), но там длительность гайдом не назначалась.

**§18, reduced-motion: тотальное выключение → выборочное.** `transition-duration:.01ms` на `*`
гасил и появление блоков: страница открывалась разом собранной и читалась как сломанная —
именно так владелец увидел её в Chrome на Android с системной настройкой «убрать анимации».
Теперь гасится движение (анимации и `transform`), а переходы прозрачности, цвета и акцента
остаются. Просьба «меньше движения» — про смещение и мельтешение, а не про то,
что интерфейс отвечает.

**§5 и §17, блюр на ≤900px.** См. §5: рецепт замены гайд уже задавал сам для `html.noblur`.

Что **не** менялось: число узлов сети (§9), радиусы блюра (§5), подъём чипа в кольце (§12),
easing, палитра, брейкпоинты.
