...
| `--hdr` | `64px` | высота шапки (в состоянии `.stuck` — 52px) |
...
**Шапка** `position:fixed`, `z-index:50`, высота `var(--hdr)` = 64px, в состоянии `.stuck`
(появляется при `scrollY > 18`) — 52px. Стекло собрано на псевдоэлементах:
...
Знак в шапке — 30px, пункты нав — 13.5px с отступом `8px 13px`.

### Отступление от макета: плотная шапка в прокрутке (04.09.2026, решение владельца)
...
Тем же решением шапка ужата: 70 → 64px, в прокрутке 60 → 52px.
...
**Знак `>jw_`** — вставляется дословно, четыре места: шапка (30px, ужат вместе с баром),
