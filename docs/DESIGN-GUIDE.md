...
Содержимое HUD в макете (`UPTIME/99.9%`) — демонстрационное. `99.9%` никто не мерил, поэтому
в проекте стоит `Цены/204 в одном файле`; остальные три пилюли — `Статус/Онлайн`,
`Стек/Python · aiogram · Docker`, `Код/8 репозиториев`. Правило «цифры только измеренные»
сильнее макета.
