...
Только системные шрифты. Ощущение «фирменного» шрифта даёт плотный отрицательный трекинг.
...
section{padding:clamp(76px,9.5vw,148px) 0;scroll-margin-top:88px}
...
(120×120, `blur(34px)`, `opacity:.4`) в правом верхнем углу. Числа считаются от нуля 1100 мс
(`easeOutCubic`) при появлении в вьюпорте; страховки — `setTimeout` на 1400 и 1500 мс ставят финальное
...
| Элемент | Позиция | `data-tz` | `data-amp` | Содержимое |
|---|---|---|---|---|
...
| `.wC` | `right:1%; bottom:0; width:min(46%,236px)` | `180` | 12 | панель метрик: `Lighthouse`, крупное `96`, спарклайн, 6 столбиков |
| `.hudA…D` | углы сцены | 200–240 | 10–12 | `STATUS/Online`, `STACK/Python · aiogram · Docker`, `LIGHTHOUSE/96`, `UPTIME/99.9%` |
...
**На ≤900px** сцена схлопывается в колонку: `.hero-l{order:-1}` — заголовок **всегда** выше сцены
(эта ошибка в проекте уже случалась), `perspective:none`, окна становятся статичными
блоками `max-width:420px`, каустики скрыты, HUD выстраивается в ряд пилюль, дрейф выключен.
...
Рядом со знаком — `.wordmark`: `jw<i>.dev</i>`, 16.5px, `letter-spacing:-.03em`, где `.dev`
приглушён до `--ink3`. Фавикон — тот же знак в `data:image/svg+xml` в `<head>`; в нём цвета
...
| Аккордеон FAQ | `max-height` `.6s` (открытый — `340px`) |
...
- **Плейсхолдеры контактов.** В макете зашиты `@jw_dev`, `hi@jw.dev` (в форме успеха, в кнопке
  копирования, в подвале, в `mailto:`). Реальность не подтверждена — заменять только по слову владельца.
