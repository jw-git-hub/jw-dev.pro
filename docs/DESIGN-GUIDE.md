...
и попросил «упростить»: плоские дорожки как на печатной плате, немного, без объёма,
и чтобы по ним шли импульсы цветами сайта.
...
Геометрия настоящей разводки: длинные прямые участки, повороты **только под 45°**,
всё на сетке `GRID = 26px` — диагональ смещается по X ровно на столько же, на сколько по Y.
Прямой участок 2–7 клеток, подъём диагонали 1–3 клетки, поворот с вероятностью `.38`.
Дорожка идёт слева направо от `x = -GRID` до края экрана, не длиннее 40 звеньев.

Плотность: `Math.min(13, Math.round(GH/90))` дорожек на десктопе и ровно 6 при `GW<760`.
Каждая начинается в своей горизонтальной полосе — иначе разводка ложится жгутом.

Цвет один на всю страницу и **вне палитры акцентов**: слейт `rgb(148,163,184)`, линия 1px,
альфа `.30`. На концах — кольцо `r=4.5` альфой `.34`, на изломах с вероятностью `.34` —
переходное отверстие `r=1.6` альфой `.40`. Ни свечения, ни градиентов: фон плоский.

**Слой статичный.** Разводка рисуется один раз в закадровый канвас при смене размера окна,
а каждый кадр только копируется одним `drawImage`. Это и есть причина, по которой цвет
дорожек не завязан на акцент: перекраска заставляла бы перерисовывать слой каждый кадр
всё время, пока цвет перетекает.

### Импульсы

Единственное, что движется, и единственное, что носит цвет секции. Импульс идёт вдоль
дорожки от начала к концу со скоростью 1,5–2,8 px за кадр; дойдя до площадки, гаснет.
Рождается не чаще одного в 900 мс и не больше пяти живых.
...
поэтому хвост повторяет её изломы, а не срезает их по прямой. Градиента вдоль пути нет
намеренно: градиент канваса живёт в экранных координатах, и его пришлось бы
пересоздавать каждый кадр для каждого импульса.

Цвет догоняет акцент секции со скоростью `tint += (target-tint)*.035` за кадр.
...
не заводится вовсе, рисуется один статичный кадр (`staticFrame()`), то есть разводка
